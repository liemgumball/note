---
Created: 2024-02-19T15:17
Class: Agility IO InternShip
Type: Back-end
Materials:
  - https://www.mongodb.com/
  - https://www.w3schools.com/mongodb/index.php
  - https://mongodb.github.io/node-mongodb-native/6.3/modules.html
Reviewed: false
Edited: 2025-05-10T14:46
---
# ==MongoDB==

> [!important] It’s a document database. It stores data in a type of
> 
...
|**SQL**|**MongoDB**|
|---|---|
|Databases are considered ==relational== ==databases==|Document database which is often referred to as a ==non-relational== database _(this does not mean that_ ==_relational data_== _cannot be stored in document databases. It means that_ ==_relational data_== _is stored differently. A better way to refer to it is as a_ ==_non-tabular database_==_.)_|
...
|When data is needed, it is queried from ==multiple tables== to join the data back together|Still have multiple groups of data too. Instead of tables these are called|
...
> [https://mongodb.github.io/node-mongodb-native/6.3/modules.html](https://mongodb.github.io/node-mongodb-native/6.3/modules.html)  
...
> [!info] Install mongosh  
> .  
> [https://www.mongodb.com/docs/mongodb-shell/install/?utm_campaign=w3schools_mdb&utm_source=w3schools&utm_medium=referral](https://www.mongodb.com/docs/mongodb-shell/install/?utm_campaign=w3schools_mdb&utm_source=w3schools&utm_medium=referral)  

Quick reference

> [!info] Quick Reference  
> See Node.  
> [https://www.mongodb.com/docs/drivers/node/current/quick-reference/#std-label-node-quick-reference](https://www.mongodb.com/docs/drivers/node/current/quick-reference/#std-label-node-quick-reference)  
...
- Projection
    ...
    > The   
    > `_id` field is also included. This field is ==always== included unless ==specifically excluded below==
...
This method ==performs batch write operations== against a _single_ collection. This method ==reduces== the number of network round trips from your application to the server which therefore increases the throughput and performance. Bulk writes return a collection of results for all operations only after ==_all_== ==operations== passed to the method complete.
...
## Aggregation Pipelines with [[NodeJS]]

> [!important] **==Aggregation operations==**
> 
> allow to `_group_`, `_sort_`, `_perform calculations_`, `_analyze data_`, and much more
...
> [!important] Don't confuse this 
> 
> `_id` expression with the `_id` ObjectId provided to each `document`
...
> [!important] This is the same
> 
> ==projection== that is used with the  method.
...
> 
> ==order== of stages matters. Each stage only acts upon the documents that previous stages provide.
...
> [!important] Using 
> 
> `$match` early in the pipeline can improve performance since it limits the number of `documents` the next stages must process.
...
> [!important] The 
> 
> `$out` stage must be the ==last== stage of the aggregation pipeline.

### **Access Data From a** `**cursor**`

> [!info] Access Data From a Cursor  
> .  
...
- **Return to Array of All** `**document**` _(Note that large numbers of matched documents can cause_ ==_performance issues_== _or_ ==_failures_== _if the operation exceeds_ ==_memory constraints_==_)_
...
In ==**MongoDB**==, multi-document transactions run within a ==**client session**==. A ==client session== is a ==grouping of related== _==read==_ ==or== _==write==_ ==operations== that you want to execute ==sequentially==

> [!info] Transactions  
> .  
> [https://www.mongodb.com/docs/drivers/node/current/fundamentals/transactions/#](https://www.mongodb.com/docs/drivers/node/current/fundamentals/transactions/#)  

> [!important] It’s recommend you reuse your client for
> 
> ==multiple sessions== and ==transactions== instead of instantiating a new client each time
...
![[Notion/Class Notes/MongoDB/attachments/Untitled.png|Untitled.png]]

> [!important] ==**Note:**==
> 
> We don’t have to create `index` for every field of data. When we add a new `document` to the `collection` , the `index` have to update its self to match that change.
