...
train_data = [list(map(int, line.strip().split(","))) for line in open("data/mnist_train.csv").read().strip().split("\n")]
...
def run_test(auto=False, no_graphics=False, validating=False):
    ...
    if validating:
        ...
        data = [list(map(int, line.strip().split(","))) for line in open("data/mnist_test.csv").read().strip().split("\n")]
    right = 0
